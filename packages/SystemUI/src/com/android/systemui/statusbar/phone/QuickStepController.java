...
public class QuickStepController implements GestureHelper {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN: {
                ...
                if (mNavigationBarView.isDt2s() && isDoubleTapPending) {
                    ...
                    if (isDoubleTapReally) HavocUtils.switchScreenOff(mContext);
                } else {
    ...
    private Runnable mDoubleTapCancelTimeout = new Runnable() {
        ...
        public void run() {
            ...
            mHandler.removeCallbacksAndMessages(null);
            HavocUtils.sendKeycode(KeyEvent.KEYCODE_HOME, mHandler);
        }
    };
    ...
    private void moveKbCursor(boolean right, boolean firstTrigger) {
        ...
        HavocUtils.moveKbCursor(KeyEvent.ACTION_UP, right);
        HavocUtils.moveKbCursor(KeyEvent.ACTION_DOWN, right);
